...
function animaAcord($acord,$pai,$filho){$(o.filho,$acord).removeClass(af);$(o.pai,$acord).removeClass(ap);$pai.addClass(ap);$filho.addClass(af);if($.isFunction(o.onAcord)){o.onAcord.apply($acord,new Array($acord,$pai,$filho,o));}
...
function terminaFocuss($t,texto){if(o.removeTexto){if($t.val()==''){$t.val(texto);}}
...
$d.delegate(elem.selector,o.evento,function(e){var $f=$(this).children(o.seletorFilho);if(!$f.length){return false;}
exibeNav($(this),$f);$(this).unbind(o.eventoFim).bind(o.eventoFim,function(){return escondeNav($(this),$f);});});function exibeNav($t,$f){$t.addClass(np);if($.isFunction(o.onExibe)){o.onExibe.apply($t,new Array($t,$f,o));}
$f.stop(o.stopClearQueue,o.stopJumpToEnd).addClass(nf)[o.efeitoIn](o.tempoIn,o.easingIn);};function escondeNav($t,$f){$t.removeClass(np);$f.stop(o.stopClearQueue,o.stopJumpToEnd).removeClass(nf)[o.efeitoOut](o.tempoOut,o.easingOut);if($.isFunction(o.onEsconde)){o.onEsconde.apply($t,new Array($t,$f,o));}};};

//Linha Slidetabs 1.1
$.fn.slideTabs=function(options){return new $.slideTabs(options,this);};$.slideTabs=function(options,elem){var padrao={seletor:'.slide',seletorAreaPainel:'.slide-conteudo',seletorPainel:'.painel',seletorMiniatura:'.miniatura',seletorAnterior:'.anterior',seletorProximo:'.proximo',classeMiniaturaAtual:'slide-miniatura-atual',classePainelAtual:'slide-painel-atual',classePainelPrimeiro:'slide-painel-primeiro',classePainelUltimo:'slide-painel-ultimo',classePainelConteudo:'slide-painel-conteudo',eventoMiniatura:'click',eventoSeta:'click',inicial:1,continuo:true,auto:false,pausa:2000,tempo:'normal',easing:'swing',alturaAutomatica:false,margin:true,scroll:1,visiveis:1,direcao:'x',onSlide:null,live:false,liveTempo:100};var o=$.extend(padrao,options),$d=$(document);if(elem===undefined){elem=$(o.seletor);}
elem.elements=[];$d.delegate(elem.selector,'iniciaSlideTabs',function(){var elems=elem.elements,els=$(elem.selector,elem.context),nEls=els.not(elems);elem.elements=els;nEls.each(function(){var $t=$(this),$sp=$(o.seletorPainel,$t),$sm=$(o.seletorMiniatura,$t),sma=o.classeMiniaturaAtual,spa=o.classePainelAtual,spp=o.classePainelPrimeiro,spu=o.classePainelUltimo,spcw=0,ini=o.inicial-1,timeout,dir=o.direcao=='x',pos={};$t.children(o.seletorAreaPainel).wrapInner('<div class="'+o.classePainelConteudo+'"></div>');var $spc=$('.'+o.classePainelConteudo,$t);$sp.each(function(i){pos[i]=spcw;spcw+=$(this)[dir?'outerWidth':'outerHeight'](o.margin);});$spc.css(dir?'width':'height',spcw).css('overflow','hidden');$spc.css(dir?'marginLeft':'marginTop',-pos[ini]);if(o.alturaAutomatica&&dir){$spc.css('height',($sp.eq(ini).outerHeight(o.margin)));}
$sp.eq(ini).addClass(spa);$sm.eq(ini).addClass(sma);if(ini==0){$sp.eq(0).addClass(spp);}
if(ini==$sp.length-1){$sp.eq(-1).addClass(spu);}
$sm.bind(o.eventoMiniatura,function(){var l=$(this).prevAll(o.seletorMiniatura).length;$sm.removeClass(sma);$sp.removeClass(spa).removeClass(spp).removeClass(spu);$(this).addClass(sma);$sp.eq(l).addClass(spa);if(l==0){$sp.eq(0).addClass(spp);}
if(l==$sp.length-1){$sp.eq(-1).addClass(spu);}
return animaSlide($(this),l);});$(o.seletorAnterior,$t).bind(o.eventoSeta,function(){var l=$('.'+spa,$t).prevAll().length-o.scroll;$sp.removeClass(spu);if(l<=0){if($sp.eq(0).hasClass(spp)){$sp.removeClass(spp);if(o.continuo){l=$sp.length-o.visiveis;$sp.eq(-1).addClass(spu);}
else{return false;}}else{l=0;$sp.eq(0).addClass(spp);}}
$sp.removeClass(spa).eq(l).addClass(spa);$sm.removeClass(sma).eq(l).addClass(sma);return animaSlide($(this),l);});$(o.seletorProximo,$t).bind(o.eventoSeta,function(){return proximoSlide($(this));});function proximoSlide($this){var l=$('.'+spa,$t).prevAll().length+o.scroll;$sp.removeClass(spp);if(($sp.length-l)<=o.visiveis){if($sp.eq(-1).hasClass(spu)){$sp.removeClass(spu);if(o.continuo){l=0;$sp.eq(0).addClass(spp);}
else{return false;}}
...
function animaSlide($this,l){if($.isFunction(o.onSlide)){o.onSlide.apply($t,new Array($t,$this,pos,l,o));}
...
$w.resize(function(e){if(atual){return posicionaTooltip(atual,e);}}).scroll(function(e){if(atual){return posicionaTooltip(atual,e);}});function criaTooltip($t,e){if($.isFunction(o.onInicia)){o.onInicia.apply($t,new Array($t,e,o));}
...
function removeTooltip($t){if($.isFunction(o.onTermina)){o.onTermina.apply($t,new Array($t,o));}
...
//Linha Valida 1.1
$.fn.valida=function(options){return new $.valida(options,this);};$.valida=function(options,elem){var padrao={seletor:'.formulario',classeValidacao:'valida',classeEmail:'email',classeUrl:'url',classeNumero:'numero',classeRegex:'regex',atributoRegex:'regex',ValorSelectPadrao:0,redireciona:true,onValida:null,onErro:null,onPassa:null,onNaoPassa:null,live:false,liveTempo:100};var o=$.extend(padrao,options),$d=$(document);if(elem===undefined){elem=$(o.seletor);}
elem.elements=[];$d.delegate(elem.selector,'iniciaValida',function(){var elems=elem.elements,els=$(elem.selector,elem.context),nEls=els.not(elems);elem.elements=els;nEls.each(function(){var $form=$(this);if(!$(this).is('form')){return false;}
$(this).find('input, button').filter(':submit').click(function(){var resultado=$.valida.form(o,nEls);if(resultado){if(!o.redireciona){if($.isFunction(o.onPassa)){o.onPassa.apply($form,new Array($form,o));}
return false;}else{return resultado;}}else{if($.isFunction(o.onNaoPassa)){o.onNaoPassa.apply($form,new Array($form,o));}
return false;}});});});if(o.live){setInterval(function(){$(elem.selector).trigger('iniciaValida');},o.liveTempo);}else{if(elem.length){elem.trigger('iniciaValida');}}};$.valida.form=function(o,elem){var passa=true;elem.each(function(){$(this).find('input, textarea, select, checkbox, radio').not(':submit').each(function(){var $t=$(this);if($t.hasClass(o.classeValidacao)){if($.isFunction(o.onValida)){o.onValida.apply(this,new Array($t,o));}
...
passa=passa&&tValido;if(!tValido){if($.isFunction(o.onErro)){o.onErro.apply(this,new Array($t,o));}}}});});return passa;};function validaInput($t,o){var val=$t.val();ret=true;if($t.hasClass(o.classeEmail)){var tmp=new RegExp(/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/);if(!tmp.test(val)||val==''){ret=false;}}
...
return ret;}
function validaSelect($t,o){if(!$t.val()||$t.val()==o.ValorSelectPadrao){return false;}
return true;}
function validaCheckboxRadio($t,o){var inputName=$t.attr('name');var checked=$("input[name="+inputName+"]:checked").length;if(checked==0){return false;}
return true;}
})(jQuery);